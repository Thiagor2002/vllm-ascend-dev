...
## PR lineage and review boundary

The development branches are layered as follows:

```text
vllm-ascend community base
  └─ mrv2_310p_ys   core 310P MRv2 adaptation and hardware bring-up
       └─ mrv2_310p_820   compatibility hardening, quantization fixes,
                          tests, and community-merge documentation
```

`mrv2_310p_ys` is not an external dependency or a separate deliverable. The
community PR consists of the core adaptation from `mrv2_310p_ys` plus the
hardening delta in `mrv2_310p_820`. Reviewers should evaluate the combined
result against the 310P MRv2 requirements below, rather than treating only the
last branch delta as the whole feature.

The original adaptation guide defined the dense/VL W8A8/W8A8SC baseline.
The combined PR expands that acceptance surface to W8A8-Dynamic, Qwen3 MoE,
Qwen3.5 dense/hybrid and MoE, embedding pooling, and explicit compatibility
without vLLM PR #43048. The support matrix below is authoritative when that
expanded scope differs from the earlier planning baseline.
...
This first release adds a 310P MRv2 stack under
`vllm_ascend/_310p/worker/v2/` and a small set of shared MRv2 extension
points. The requested acceptance surface is:
...
and are deferred to a follow-up 310P MRv2 release. Disabling
`VLLM_USE_V2_MODEL_RUNNER` must leave V1 behavior unchanged.

## 2. Change structure

```text
Core adaptation (mrv2_310p_ys, included in this PR)
...
Hardening (mrv2_310p_820)
  _310p/worker/v2/block_table.py
                                direct NumPy slot mapping from CPU mirrors
  _310p/worker/v2/feature_support.py
                                immutable first-release capability boundary
  _310p/worker/v2/kernel_registry.py
                                optional future dispatcher bridge
  worker/v2/block_table.py      restore the existing multi-group Triton ABI
                                for non-310P devices
